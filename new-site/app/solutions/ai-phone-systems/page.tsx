        ...
        <div className="grid lg:grid-cols-2 gap-8 sm:gap-12 lg:gap-16 items-center">
          ...
          <FadeInSection>
            <div className="space-y-5 sm:space-y-6 md:space-y-8">
              {/* Badge */}
              <div className="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-full border border-brand-primary/30 bg-brand-primary/10">
                <Phone className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-brand-primary" />
              ...
              {/* Trust Points */}
              <div className="flex flex-wrap gap-4 sm:gap-6 text-xs sm:text-sm text-zinc-400">
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                <Link href="#demo" className="w-full sm:w-auto">
                  <motion.div
                    data-magnetic
                    className="inline-flex items-center justify-center gap-2 sm:gap-3 w-full sm:w-auto px-6 sm:px-8 py-3 sm:py-4 bg-gradient-to-r from-brand-primary to-brand-secondary text-white rounded-full font-semibold text-base sm:text-lg cursor-pointer hover:shadow-brand-lg transition-all duration-300"
                ...
                <Link href="/quote" className="w-full sm:w-auto">
                  <motion.div
                    className="inline-flex items-center justify-center gap-2 sm:gap-3 w-full sm:w-auto px-6 sm:px-8 py-3 sm:py-4 border border-zinc-700 text-white rounded-full font-semibold text-base sm:text-lg cursor-pointer hover:bg-zinc-800/50 transition-all duration-300"
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                  >
...
function VoiceVisualization({ activeScenario, onScenarioClick }: VoiceVisualizationProps) {
  ...
  return (
    <div className="relative">
      ...
      <div className="relative bg-zinc-900/80 backdrop-blur-xl rounded-2xl sm:rounded-3xl border border-zinc-800 p-4 sm:p-6 overflow-hidden">
        {/* Ambient glow */}
        <div className={cn(
          ...
          {isProcessing && (
            ...
            >
              <div className="flex items-center gap-2 sm:gap-3 mb-2">
                <Brain className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-brand-primary animate-pulse" />
                <span className="text-xs sm:text-sm text-zinc-400">Processing Intent...</span>
              </div>
        ...
        <div className="relative space-y-3 sm:space-y-4 min-h-[180px] sm:min-h-[200px]">
                ...
                >
                  {message.type === 'caller' && (
                    <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full bg-zinc-700 flex items-center justify-center flex-shrink-0">
                      <Mic className="w-3 h-3 sm:w-4 sm:h-4 text-zinc-300" />
                    </div>
                  )}
                  <div className={cn(
                    "max-w-[85%] sm:max-w-[80%] px-3 sm:px-4 py-2 sm:py-3 rounded-xl sm:rounded-2xl text-sm sm:text-base",
                    message.type === 'caller'
        ...
        {/* Scenario Tabs */}
        <div className="relative mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-zinc-800">
            ...
            {scenarios.map((s, index) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "flex-1 flex items-center justify-center gap-1 sm:gap-2 px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg sm:rounded-xl text-xs sm:text-sm font-medium transition-all duration-300",
      ...
      {/* Floating particles around the visualization - Hidden on mobile */}
...
function AnimatedWaveform({ isActive, color }: { isActive: boolean; color: string }) {
  const bars = 24; // Reduced from 32 for better mobile rendering

  return (
    <div className="flex items-center justify-center gap-[2px] h-12 sm:h-16">
      {Array.from({ length: bars }).map((_, i) => (
        <motion.div
          key={i}
          className={cn("w-0.5 sm:w-1 rounded-full bg-gradient-to-t", color)}
          animate={{
            height: isActive
              ? [6, Math.random() * 36 + 12, 6]
              : 6,
  ...
  return (
    <section className="relative py-16 sm:py-20 md:py-24 overflow-hidden">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {stepsWithFeatures.map((s, index) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "group relative flex items-center gap-2 sm:gap-3 px-3 sm:px-5 py-2 sm:py-3 rounded-lg sm:rounded-xl font-medium transition-all duration-300",
                    isActive
                ...
                >
                  {/* Step number */}
                  <span className={cn(
                    "w-6 h-6 sm:w-7 sm:h-7 rounded-md sm:rounded-lg flex items-center justify-center text-xs sm:text-sm font-bold transition-all duration-300",
                    isActive
                  ...
                  {isActive && (
                    <motion.div
                      layoutId="activeTab"
                      className={cn(
                        "absolute inset-0 rounded-lg sm:rounded-xl border-2 -z-10",
        ...
        {/* Content Area */}
        <FadeInSection delay={0.2}>
          <div className="relative rounded-xl sm:rounded-2xl border border-zinc-800 bg-zinc-900/50 overflow-hidden">
            {/* Background glow */}
            <motion.div
              className={cn(
            ...
            {/* Header */}
            <div className="flex items-center gap-3 sm:gap-4 p-4 sm:p-6 border-b border-zinc-800/50">
              <div className={cn(
                  ...
                  {step.features.map((feature, index) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className="group p-4 sm:p-5 rounded-lg sm:rounded-xl bg-zinc-800/30 border border-zinc-700/50 hover:border-zinc-600 hover:bg-zinc-800/50 transition-all duration-300"
                      >
                        <div className={cn(
                          "w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl flex items-center justify-center mb-2 sm:mb-3 transition-all duration-300",
                          "bg-zinc-700/50 group-hover:bg-gradient-to-br",
                          `group-hover:${step.color}`
                        )}>
                          <FeatureIcon className="w-4 h-4 sm:w-5 sm:h-5 text-zinc-300 group-hover:text-white transition-colors" />
                        </div>
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      index === activeStep
                        ...
                        : 'w-6 sm:w-8 bg-zinc-700 hover:bg-zinc-600'
                    )}
                  />
                ))}
              </div>
  ...
  return (
    <section className="relative py-16 sm:py-24 md:py-32 overflow-hidden">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <FadeInSection delay={0.1}>
          <div className="relative overflow-x-auto rounded-xl sm:rounded-2xl border border-zinc-800">
            {/* Table Header */}
            <div className="grid grid-cols-3 bg-zinc-900/80 min-w-[500px] sm:min-w-0">
              <div className="p-3 sm:p-4 text-xs sm:text-sm md:text-base text-zinc-400 font-medium border-b border-zinc-800">
                Feature
              </div>
              <div className="p-3 sm:p-4 text-center text-xs sm:text-sm md:text-base text-zinc-400 font-medium border-b border-zinc-800">
                Traditional
              </div>
              <div className="p-3 sm:p-4 text-center text-xs sm:text-sm md:text-base font-medium border-b border-zinc-800 bg-gradient-to-r from-brand-primary/20 to-cyan-500/20">
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-primary to-cyan-400">
                  AI Phone
                </span>
              </div>
            ...
            <div className="min-w-[500px] sm:min-w-0">
              {comparisonData.map((row, index) => (
                ...
                >
                  <div className="p-3 sm:p-4 text-xs sm:text-sm md:text-base text-zinc-300 flex items-center gap-1.5 sm:gap-2">
                    {row.highlight && (
                      <Sparkles className="w-3 h-3 sm:w-4 sm:h-4 text-amber-400 flex-shrink-0" />
                    )}
                    <span>{row.feature}</span>
                  </div>
                  <div className="p-3 sm:p-4 text-center text-xs sm:text-sm md:text-base text-zinc-500">
                    {row.traditional}
                  </div>
                  <div className="p-3 sm:p-4 text-center bg-gradient-to-r from-brand-primary/5 to-cyan-500/5">
                    <span className="inline-flex items-center gap-1 sm:gap-2 text-xs sm:text-sm md:text-base text-emerald-400 font-medium">
          ...
          {useCases.map((useCase, index) => {
            ...
            return (
              <FadeInSection key={useCase.title} delay={index * 0.05}>
                <motion.div
                  className="relative group p-4 sm:p-6 rounded-xl sm:rounded-2xl border border-zinc-800 bg-zinc-900/50 cursor-pointer overflow-hidden"
                ...
                >
                  ...
                  <div className="relative z-10">
                    {/* Icon */}
                    <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl bg-zinc-800 flex items-center justify-center mb-3 sm:mb-4 group-hover:bg-gradient-to-r group-hover:from-brand-primary group-hover:to-cyan-500 transition-all duration-300">
                      <Icon className="w-5 h-5 sm:w-7 sm:h-7 text-brand-primary group-hover:text-white transition-colors duration-300" />
                    </div>
                    ...
                    {/* Example conversation - always visible on mobile, hover on desktop */}
                    <div className="sm:hidden pt-3 border-t border-zinc-800">
                      <p className="text-xs text-zinc-500 italic">
                        {useCase.example}
                      </p>
                    </div>

                    {/* Example conversation on hover (desktop only) */}
                    <AnimatePresence>
                      {isHovered && (
                        <motion.div
        ...
        <div className="space-y-2 sm:space-y-3">
          {faqs.map((faq, index) => (
            <FadeInSection key={index} delay={index * 0.03}>
              <motion.div
                className="border border-zinc-800 rounded-lg sm:rounded-xl overflow-hidden bg-zinc-900/50"
                initial={false}
              >
                <button
                ...
                >
                  <span className="text-sm sm:text-base md:text-lg font-medium text-white pr-3 sm:pr-4">
                    {faq.question}
                  </span>
                  <motion.div
                ...
                <AnimatePresence initial={false}>
                  {openIndex === index && (
                    ...
                    >
                      <div className="px-4 sm:px-5 pb-4 sm:pb-5 text-sm sm:text-base text-zinc-400 border-t border-zinc-800 pt-3 sm:pt-4">
                        {faq.answer}
                      </div>
                    </motion.div>
                  )}
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <FadeInSection>
          ...
          <Link href="/quote">
            <motion.div
              data-magnetic
              className="inline-flex items-center gap-2 sm:gap-3 px-6 sm:px-8 md:px-10 py-3.5 sm:py-4 md:py-5 bg-gradient-to-r from-brand-primary to-brand-secondary text-white rounded-full font-semibold text-base sm:text-lg md:text-xl cursor-pointer hover:shadow-brand-lg transition-all duration-300"
          ...
          {/* Trust points */}
          <div className="flex flex-col sm:flex-row flex-wrap justify-center gap-3 sm:gap-6 md:gap-8 mt-8 sm:mt-10 md:mt-12 text-xs sm:text-sm md:text-base text-zinc-500">
