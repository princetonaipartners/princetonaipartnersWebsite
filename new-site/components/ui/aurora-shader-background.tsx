...
interface AuroraShaderBackgroundProps {
  ...
  speed?: number;
  /** Opacity value (0-1) - can be a MotionValue for scroll-based control */
...
export function AuroraShaderBackground({
        ...
        void main() {
          ...
          for (float i = 0.0; i < 35.0; i++) {
            ...
            vec4 blueColors = vec4(
              0.0,                                           // No red
              0.4 + 0.4 * cos(i * 0.25 + iTime * 0.4),      // Cyan component
              0.8 + 0.2 * sin(i * 0.3 + iTime * 0.3),       // Strong blue
              1.0
            );
  ...
  const opacityMotion = typeof opacity !== 'number' ? opacity : undefined;
  ...
  return (
    <motion.div
      ...
      style={{
        opacity: opacityMotion ?? opacityValue,
        zIndex: 0,
      }}
    />
  );
