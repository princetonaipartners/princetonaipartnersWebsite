...
export function DottedSurface({ className, children, ...props }: DottedSurfaceProps) {
  ...
  useEffect(() => {
    ...
    scene.fog = new THREE.Fog(0x09090b, 2000, 10000); // zinc-950 fog
    ...
    for (let ix = 0; ix < AMOUNTX; ix++) {
      for (let iy = 0; iy < AMOUNTY; iy++) {
        ...
        // Cyan color for dark theme (matching web scraping page accent)
        if (theme === 'dark' || !theme) {
          colors.push(0.024, 0.714, 0.831); // #06B6D4 cyan
        } else {
          colors.push(0, 0, 0);
        }
      }
