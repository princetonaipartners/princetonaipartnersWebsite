...
const FloatingAiAssistant = () => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      ...
      <button
        className={`floating-ai-button relative w-16 h-16 rounded-full flex items-center justify-center transition-all duration-500 transform hover:scale-110 hover:rotate-[5deg] ${
          isChatOpen ? 'rotate-90' : 'rotate-0'
        }`}
        onClick={() => setIsChatOpen(!isChatOpen)}
        style={{
          background:
            'linear-gradient(135deg, rgba(10,132,255,0.9) 0%, rgba(0,102,204,0.9) 100%)',
          boxShadow:
            '0 0 20px rgba(10, 132, 255, 0.7), 0 0 40px rgba(0, 102, 204, 0.5), 0 0 60px rgba(10, 132, 255, 0.3)',
      ...
      >
        ...
        {/* Glowing animation */}
        <div className="absolute inset-0 rounded-full animate-ping opacity-20 bg-brand-primary"></div>
      </button>
      ...
      {isChatOpen && (
        <div
          ref={chatRef}
          className="absolute bottom-20 right-0 w-max max-w-[500px] transition-all duration-300 origin-bottom-right animate-in fade-in zoom-in-95"
        >
